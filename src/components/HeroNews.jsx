    ...
    <div className='flex flex-col lg:flex-row lg:p-5 dark:text-gray-300'>
        ...
        className='relative cursor-pointer group overflow-hidden w-full'>
            {articles.slice(0, 1).map((article) => (
                <div>
                    <img loading='lazy' className='w-full lg:h-140 object-cover lg:rounded-xl' src={article.image_url} alt="Hero-news" />

                    <div className='absolute inset-0 bg-linear-to-t from-black via-black/40 to-transparent lg:h-140 lg:rounded-xl'></div>

                    <h1 className='absolute inset-0 p-5 text-4xl md:text-6xl md:py-10 text-gray-100 dark:text-gray-200 font-bold'>Featured News</h1>

                <div className='absolute bottom-0 md:bottom-10 lg:bottom-5 text-white p-5 flex flex-col gap-2 md:gap-6'>
                
                    <p className='text-xs text-gray-200 bg-gray-200/30 w-30 text-center py-1 rounded-full'>{article.category}</p>
                    {/* <h1 className='text-2xl md:text-5xl font-bold md:leading-15 dark:text-gray-200'>{article.title}</h1> */}
                    <h2 className='text-2xl md:text-3xl dark:text-gray-200 font-bold'>{article.title}</h2>
        ...
        {articles.slice(2, 6).map((article) => (
            <div className='flex flex-col border-b border-gray-300 hover:scale-103 transition-all duration-500 cursor-pointer'>
                <div className='p-5 flex gap-3'>
                    <img className='w-25 h-25 object-cover rounded-xl' src={article.image_url} alt="" />
